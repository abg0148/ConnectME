...
try:
    with open("user_pass_data_4.txt","rb") as fp:
...
try:
    with open("weight_data_4.txt","rb") as fp:
...
r = Tk()
r.configure(background = "#fcf594")
r.title('People Graph')
r.geometry('300x300')


intruction = Label(r, text='SIGN UP', bg ="#fcf594").grid(row = 0, column = 1)
#intruction.pack()
nameL = Label(r, text='New Username: ',bg ="#fcf594").grid(row = 1) 
passwordL = Label(r, text='New Password: ',bg ="#fcf594").grid(row = 2)

nameE = Entry(r).grid(row = 1, column = 1)
passwordE = Entry(r, show='*').grid(row = 2, column = 1)
#nameE.pack()
#passwordE.pack() 
signupButton = Button(r, text='Signup', width = 5, command = register).grid(row = 3)
#signupButton.pack()


loginInstruction = Label(r, text='LOG IN\n', bg ="#fcf594").grid(row = 4, column = 1, columnspan = 2)
#loginInstruction.pack()
nameL = Label(r, text='Username: ',bg ="#fcf594" ).grid(row = 5)
pwordL = Label(r, text='Password: ', bg ="#fcf594").grid(row = 6)
#nameL.pack() 
#pwordL.pack() 
 
nameLogin = Entry(r).grid(row = 5, column = 1)
pwordLogin = Entry(r, show='*').grid(row = 6, column = 1)
#nameLogin.pack()
#pwordLogin.pack() 
 
loginButton = Button(r, text='Login', width = 5, command = afterLogin).grid(row = 12)
#loginButton.pack()

def endingFunction():
    # save user_data
    with open("user_pass_data_4.txt","wb") as fp:
        pickle.dump(user_data,fp)
        
    li=[(u,v,d) for (u,v,d) in G.edges(data=True)]
    #save list
    with open("weight_data_4.txt","wb") as fp:
        pickle.dump(li,fp)
